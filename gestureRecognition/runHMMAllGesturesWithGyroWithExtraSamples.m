...
load('C:\Users\Dick\Dropbox\School\UF\2014-1 Spring\EEL6825 Pattern Recognition\Project\Programs\Data\test2\gestureDataWithNoise.mat');
load('C:\Users\Dick\Dropbox\School\UF\2014-1 Spring\EEL6825 Pattern Recognition\Project\Programs\Data\test2\OthersGestureDataWithNoise.mat')
...
for i = 1:Q
    ...
    eval( ['[ loglik' num2str(i) ', LL' num2str(i) ', prior' num2str(i) ', transmat' num2str(i) ', mu' num2str(i) ', Sigma' num2str(i) ', mixmat' num2str(i) ' ] = GenMHMM( ' Motions{i} '_data(:,:,[1:numTrain numTrain+numTest+1:50]),O,M,Q );'] );
...
for i = [numTrain+1:numTrain+numTest]
    ...
    for j = 1:Q
        ...
        for k = 1:Q
        eval(['LL(' num2str(k) ') = mhmm_logprob(' Motions{j} '_data(:,:,' num2str(i) '), prior' num2str(k) ', transmat' num2str(k) ', mu' num2str(k) ', Sigma' num2str(k) ', mixmat' num2str(k) ');']);
 ...
 missclass(1) = ( sum(sum(Confusion))-sum(diag(Confusion)) )/sum(sum(Confusion));
...
for i = 1:10
  ...
  missclass(2) = ( sum(sum(Confusion2))-sum(diag(Confusion2)) )/sum(sum(Confusion2));
...
%% Look at the results of all user gestures (both users)
   ...
   missclass(3) = ( sum(sum(ConfusionTot))-sum(diag(ConfusionTot)) )/sum(sum(ConfusionTot));
   missclass = missclass*100
